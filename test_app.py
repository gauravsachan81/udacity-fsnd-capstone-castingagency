...
class ActorsTestCase(unittest.TestCase):
    ...
    def test_get_actors_negative(self):
        response = self.client.get("/actors", headers={"Authorization": "Bearer {}".format(self.producer)})
    ...
    def test_post_an_actor(self):
        test_actor = {"name": "Sachan", "age": 42, "gender": "male"}
        print("test_actor set")
        response = self.client.post("/actors", headers={"Authorization": "Bearer {}".format(self.producer)}, json=test_actor)
        print("response set")
        response_data = json.loads(response.data)
        ...
        test_actor["actor_id"] = response_data['actors']['actor_id']
        self.assertEqual(i_actor.serialized_actor(), test_actor, "Actor in test case and the actor posted in DB are not same")

    # Get Test Case for Actors #
    def test_get_actors_postitive(self):
        test_actor = {"name": "VK", "age": 36, "gender": "male"}
        response = self.client.post("/actors", headers={"Authorization": "Bearer {}".format(self.producer)}, json=test_actor)
        response = self.client.get("/actors", headers={"Authorization": "Bearer {}".format(self.producer)})
        response_data = json.loads(response.data)
        self.assertTrue(response_data['success'], "Success attribute of response json = False")
    ...
    def test_post_an_actor_negative(self):
        test_actor = {"age": 24, "gender": "female"}
        response = self.client.post("/actors", headers={"Authorization": "Bearer {}".format(self.producer)}, json=test_actor)
        response_data = json.loads(response.data)
    ...
    def test_patch_an_actor(self):
        test_actor = {"name": "GS", "age": 42, "gender": "male"}
        response = self.client.post("/actors", headers={"Authorization": "Bearer {}".format(self.producer)}, json=test_actor)
        actor_id = json.loads(response.data)['actors']['actor_id']
        test_actor_upd = {"name": "Sachan"}
        response_upd = self.client.patch(f'/actors/{actor_id}', json=test_actor_upd, headers={"Authorization": "Bearer {}".format(self.producer)})
        ...
        test_actor["actor_id"] = actor_id
        self.assertEqual(response_data["actor"], test_actor, "Actor in test case and the actor posted in DB are not same")
    ...
    def test_delete_an_actor(self):
        test_actor = {"name": "GS", "age": 42, "gender": "male"}
        response = self.client.post("/actors", json=test_actor, headers={"Authorization": "Bearer {}".format(self.producer)})
        actor_id = json.loads(response.data)['actors']['actor_id']
        response_upd = self.client.delete(f'/actors/{actor_id}', headers={"Authorization": "Bearer {}".format(self.producer)})
        response_data = json.loads(response_upd.data)
        self.assertTrue(response_data['success'], "success attribute in response json was false")
...
class MoviesTestCase(unittest.TestCase):
    ...
    def test_get_movies_postitive(self):
        test_movie = {"movie_title": "Terinator", "release_date": "2022-12-22"}
        response = self.client.post("/movies", json=test_movie, headers={"Authorization": "Bearer {}".format(self.producer)})
        response = self.client.get("/movies", headers={"Authorization": "Bearer {}".format(self.producer)})
        response_data = json.loads(response.data)
        self.assertTrue(response_data['success'], "success attribute in response json was false")
    ...
    def test_post_a_movies(self):
        test_movie = {"movie_title": "Terminator", "release_date": "2022-12-22"}
        response = self.client.post("/movies", json=test_movie, headers={"Authorization": "Bearer {}".format(self.producer)})
        response_data = json.loads(response.data)
        self.assertTrue(response_data['success'], "success attribute in response json was false")
        i_movie = Movie.query.get(response_data['movies']['movie_id'])
        test_movie["movie_id"] = response_data['movies']['movie_id']
        self.assertEqual(i_movie.serialized_movie(), test_movie, "Movie in test case and the movie posted in DB are not same")

    # Post Test Case for Movies - Negative #
    def test_post_a_movie_negative(self):
        test_movie = {"movie_title": "Terminator"}
        response = self.client.post("/movies", json=test_movie, headers={"Authorization": "Bearer {}".format(self.producer)})
        response_data = json.loads(response.data)
    ...
    def test_patch_a_movie(self):
        ...
        test_movie_upd = {"movie_title": "Terminal"}
        response_upd = self.client.patch(f'/movies/{movie_id}', json=test_movie_upd, headers={"Authorization": "Bearer {}".format(self.producer)})
        response_data = json.loads(response_upd.data)
        self.assertTrue(response_data['success'], "success attribute in response json was false")
        test_movie["movie_title"] = test_movie_upd["movie_title"]
        test_movie["movie_id"] = movie_id
        self.assertEqual(response_data["movie"], test_movie, "Movie in test case and the movie posted in DB are not same")

    # Update Test Case for Movies - Negative #
    def test_patch_a_movie_negative(self):
        movie_id = 99999
        test_movie_upd = {"movie_title": "The Terminal"}
        response_upd = self.client.patch(f'/movies/{movie_id}', json=test_movie_upd, headers={"Authorization": "Bearer {}".format(self.producer)})
        response_data = json.loads(response_upd.data)
        self.assertFalse(response_data['success'], "success attribute in response json was true")
        self.assertEqual(int(response_data['error']), 404, "error code is not 404")
    ...
    def test_delete_a_movie(self):
        ...
        response = self.client.post("/movies", json=test_movie, headers={"Authorization": "Bearer {}".format(self.producer)})
        movie_id = json.loads(response.data)['movies']['movie_id']
        response_upd = self.client.delete(f'/movies/{movie_id}', headers={"Authorization": "Bearer {}".format(self.producer)})
        response_data = json.loads(response_upd.data)
        self.assertTrue(response_data['success'], "success attribute in response json was false")
...
class AuthTestCase(unittest.TestCase):
    ...
    def test_post_a_movies_assisstant(self):
        test_movie = {"movie_title": "Terminator", "release_date": "2022-12-22"}
        response = self.client.post("/movies", json=test_movie, headers={"Authorization": "Bearer {}".format(self.assistant)})
        response_data = json.loads(response.data)
        self.assertFalse(response_data['success'], "success attribute in response json was True")
    ...
    def test_delete_a_movie_assisstant(self):
        ...
        response = self.client.post("/movies", json=test_movie, headers={"Authorization": "Bearer {}".format(self.producer)})
        movie_id = json.loads(response.data)['movies']['movie_id']
        response_upd = self.client.delete(f'/movies/{movie_id}', headers={"Authorization": "Bearer {}".format(self.assistant)})
        response_data = json.loads(response_upd.data)
        self.assertFalse(response_data['success'], "success attribute in response json was True")
        self.assertEqual(int(response_data['error']), 403, "error code is not 403")
    ...
    def test_post_a_movies_director(self):
        test_movie = {"movie_title": "Terminator", "release_date": "2022-12-22"}
        response = self.client.post("/movies", json=test_movie, headers={"Authorization": "Bearer {}".format(self.director)})
        response_data = json.loads(response.data)
        self.assertFalse(response_data['success'], "success attribute in response json was True")
        self.assertEqual(int(response_data['error']), 403, "error code is not 403")
    ...
    def test_delete_a_movie_director(self):
        ...
        response = self.client.post("/movies", json=test_movie, headers={"Authorization": "Bearer {}".format(self.producer)})
        movie_id = json.loads(response.data)['movies']['movie_id']
        response_upd = self.client.delete(f'/movies/{movie_id}', headers={"Authorization": "Bearer {}".format(self.director)})
        response_data = json.loads(response_upd.data)
        self.assertFalse(response_data['success'], "success attribute in response json was True")
        self.assertEqual(int(response_data['error']), 403, "error code is not 403")
