...
def get_token_auth_header():
    ...
    if head is None:
        raise AuthError({'status':'Auth_Header_Not_Available','description':'Authorization Header should be present.'},401)
    ...
    if auth[0].upper() != 'BEARER':
        raise AuthError({
            ...
            'description':'Bearer should be included in the Authorization header'
        },401)
    ...
    # Checking if request has token in the auth header
        ...
        raise AuthError({
            ...
            'description':'Authorization Token should be included in the Authorization header'
        },401)
    # Returning the token
    return auth[1]

def check_permissions(permission, payload):
    # Validating correct persmissions in the Json Web Token to implement Role Based Acces Control (RBAC)
...
def verify_decode_jwt(token):
    ...
    if 'kid' not in head:
        raise AuthError({
            ...
            'description' : 'Kid is not present in the JWT header'
        },401)
